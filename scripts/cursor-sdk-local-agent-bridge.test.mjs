...
import {
  extractComposerToolOutput,
  parseComposerToolCalls,
} from "./composer-tool-markers.mjs";

const bridgeScriptPath = fileURLToPath(
  new URL("./cursor-sdk-local-agent-bridge.mjs", import.meta.url),
);
...
describe("Cursor SDK local-agent bridge", () => {
  ...
  it("classifies retryable Cursor SDK upstream capacity errors", () => {
    ...
    expect(
      isRetryableSDKRunError({ rawMessage: "temporarily unavailable" }),
    ).toBe(true);
    expect(isRetryableSDKRunError({ status: 429 })).toBe(true);
    expect(
      isRetryableSDKRunError(new Error("Missing or invalid authorization")),
    ).toBe(false);
    expect(
      isRetryableSDKRunError({ status: 401, message: "Unauthorized" }),
    ).toBe(false);
  });
  ...
  it("treats opaque SDK error results as retryable but preserves explicit auth failures", () => {
    ...
    expect(
      sdkRunFailureSummary({
        status: "error",
        error: { message: "Server at capacity", code: "unavailable" },
      }),
    ).toMatchObject({
  ...
  it("keeps public Composer aliases distinct before SDK model selection", () => {
    ...
    expect(normalizeModel("cursorapi/composer-2.5-fast")).toBe(
      "composer-2.5-fast",
    );
  ...
  it("treats ConnectRPC cancel errors as benign aborts", () => {
    ...
    expect(isBenignCancellationError(new Error("upstream unavailable"))).toBe(
      false,
    );
  });
  ...
  it("parses embedded Composer tool-call markup into structured tool calls", () => {
    ...
    expect(parseComposerToolCalls(malformedKimi)).toEqual([
      {
        name: "search_files",
        arguments: { pattern: "*", target: "files", limit: 50 },
      },
    ]);
  });
  ...
  it("serializes overlapping runs for the same stateful SDK agent", async () => {
    ...
    await firstStarted;
    const second = runExclusiveForAgent(
  ...
  it("allows different SDK sessions to run concurrently", async () => {
    ...
    const order = [];
    let releaseFirst;
    const first = runExclusiveForAgent(
      { ...baseInput, sessionKey: "session-a" },
      async () => {
        order.push("first:start");
        await new Promise((release) => {
          releaseFirst = release;
    ...
    const second = runExclusiveForAgent(
      { ...baseInput, sessionKey: "session-b" },
      async () => {
        order.push("second:start");
        return "second";
      },
  ...
  it("allows SDK glob calls once a real pattern is present", () => {
    expect(
      isForwardableSDKToolCall({
        name: "glob",
        arguments: { globPattern: "**/*.tsx", targetDirectory: "." },
      }),
    ).toBe(true);
    expect(
      isForwardableSDKToolCall({
        name: "glob",
        arguments: { glob_pattern: "*.tsx", targeting: "src" },
      }),
    ).toBe(true);
    expect(
      isForwardableSDKToolCall({
        name: "glob",
        arguments: { targeting: "/tmp/project/src/**/*.tsx" },
      }),
    ).toBe(true);
  });
  ...
  it("requires both provider and tool names for SDK MCP forwarding", () => {
    expect(
      isForwardableSDKToolCall({
        name: "mcp",
        arguments: { providerIdentifier: "client" },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
        name: "mcp",
        arguments: { providerIdentifier: "client", toolName: "glob" },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
  ...
  it("requires complete non-file SDK builtin tool arguments before forwarding", () => {
    expect(
      isForwardableSDKToolCall({ name: "readLints", arguments: { cwd: "." } }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({ name: "readLints", arguments: { paths: [] } }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
        name: "readLints",
    ...
    expect(
      isForwardableSDKToolCall({
        name: "readLints",
        arguments: { filePath: "src/App.tsx" },
      }),
    ).toBe(true);
    ...
    expect(
      isForwardableSDKToolCall({
        name: "semSearch",
        arguments: { targetDirectories: ["src"] },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
        name: "semSearch",
        arguments: { query: "submit button" },
      }),
    ).toBe(true);
    expect(
      isForwardableSDKToolCall({
    ...
    expect(
      isForwardableSDKToolCall({ name: "todowrite", arguments: { todos: [] } }),
    ).toBe(true);
    expect(
      isForwardableSDKToolCall({
        name: "todowrite",
        arguments: { taskList: [{ content: "Build app" }] },
      }),
    ).toBe(true);
    ...
    expect(
      isForwardableSDKToolCall({
        name: "task",
        arguments: { description: "Inspect app" },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
    ...
    expect(
      isForwardableSDKToolCall({
        name: "createPlan",
        arguments: { plan: "Build and verify the app" },
      }),
    ).toBe(true);
    expect(
      isForwardableSDKToolCall({
        name: "createPlan",
        arguments: { todos: [{ content: "Build app" }] },
      }),
    ).toBe(true);
    ...
    expect(
      isForwardableSDKToolCall({
        name: "generateImage",
        arguments: { filePath: "out.png" },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
        name: "generateImage",
        arguments: { description: "A blue cube" },
      }),
    ).toBe(true);
    ...
    expect(
      isForwardableSDKToolCall({
        name: "recordScreen",
        arguments: { path: "recording.mov" },
      }),
    ).toBe(false);
    expect(
      isForwardableSDKToolCall({
        name: "recordScreen",
        arguments: { mode: "START_RECORDING" },
      }),
    ).toBe(true);
  });
  ...
  it("exposes dynamic client tool schemas through the forwarding MCP server", () => {
    ...
    expect(
      tools.find((tool) => tool.name === "probe_write_file"),
    ).toMatchObject({
      description: "Writes a marker through the harness MCP server.",
      inputSchema: {
        additionalProperties: false,
    ...
    expect(
      tools.map((tool) => tool.name).indexOf("probe_write_file"),
    ).toBeLessThan(tools.map((tool) => tool.name).indexOf("client_shell"));
  });
  ...
  it("unwraps common dynamic client tool schema wrappers", () => {
    ...
    expect(
      tools.find((tool) => tool.name === "wrapped_write_file"),
    ).toMatchObject({
      inputSchema: {
        type: "object",
        additionalProperties: false,
    ...
    expect(
      validateClientMcpToolCall(tools, "wrapped_write_file", {
        path: "marker.txt",
      }),
    ).toBe("Missing required argument for wrapped_write_file: content");
    expect(
      validateClientMcpToolCall(tools, "wrapped_write_file", {
        path: "marker.txt",
        content: "ok",
  ...
  it("rejects unknown or incomplete client MCP forwarding calls internally", () => {
    ...
    expect(validateClientMcpToolCall(tools, "missing_tool", {})).toContain(
      "Unknown client MCP forwarding tool",
    );
    expect(
      validateClientMcpToolCall(tools, "probe_write_file", {
        file_path: "marker.txt",
      }),
    ).toBe("Missing required argument for probe_write_file: contents");
    expect(
      validateClientMcpToolCall(tools, "probe_write_file", {
        file_path: "marker.txt",
        contents: "",
  ...
  it("validates dynamic client MCP schemas with local references", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "ref_write_file", {
    ...
    expect(
      ...
      "Invalid value for ref_write_file.metadata.tags[0]: expected string",
    );
    expect(
      validateClientMcpToolCall(tools, "ref_write_file", {
        target: { path: "src/App.tsx", mode: "create" },
  ...
  it("validates dynamic client MCP scalar constraints", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "",
        limit: 5,
      }),
    ).toBe(
      "Invalid value for constrained_search.query: expected at least 2 character(s)",
    );
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo app",
    ...
    ).toContain("expected to match pattern");
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo_app_name",
        limit: 5,
      ...
      "Invalid value for constrained_search.query: expected at most 12 character(s)",
    );
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo_app",
    ...
    ).toBe("Invalid value for constrained_search.limit: expected >= 1");
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo_app",
        limit: 55,
      }),
    ).toBe("Invalid value for constrained_search.limit: expected <= 50");
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo_app",
        limit: 7,
      ...
      "Invalid value for constrained_search.limit: expected a multiple of 5",
    );
    expect(
      validateClientMcpToolCall(tools, "constrained_search", {
        query: "todo_app",
  ...
  it("validates dynamic client MCP array membership constraints", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "run_steps", {
    ...
    expect(
      ...
      "Invalid value for run_steps.steps: expected at least 1 matching item",
    );
    expect(
      validateClientMcpToolCall(tools, "run_steps", {
    ...
    expect(
      ...
      "Invalid value for run_steps.labels: expected at most 1 matching item",
    );
  });
  ...
  it("validates dynamic client MCP tuple array schemas", () => {
    const tools = clientMcpToolDefinitions([
      {
        ...
        parameters: {
          ...
          properties: {
            steps: {
              type: "array",
              items: [{ const: "install" }, { const: "build" }],
    ...
    expect(
      validateClientMcpToolCall(tools, "run_pipeline", {
        steps: ["install", "build"],
      }),
    ).toBe(null);
    expect(
      ...
      'Invalid value for run_pipeline.steps[1]: expected constant "build"',
    );
    expect(
      validateClientMcpToolCall(tools, "run_pipeline", {
        steps: ["install", "build", "test"],
  ...
  it("validates dynamic client MCP unevaluated object properties", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "configure_deploy", {
  ...
  it("treats additionalProperties schemas as evaluated for unevaluated object properties", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "configure_env", {
        command: "npm run build",
        NODE_ENV: "production",
      }),
    ).toBe(null);
    expect(
      ...
      "Invalid value for configure_env.NODE_ENV: expected at least 1 character(s)",
    );
    expect(
      validateClientMcpToolCall(tools, "configure_env", {
        command: "npm run build",
  ...
  it("treats dependentSchemas as evaluated for unevaluated object properties", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "run_command", {
        command: "npm run build",
        cwd: ".",
      }),
    ).toBe(null);
  ...
  it("validates dynamic client MCP oneOf schemas exactly", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "deploy_target", {
        target: { path: "dist" },
      }),
    ).toBe(null);
    expect(
      ...
      "Invalid value for deploy_target.target: matched more than one allowed schema",
    );
    expect(
      validateClientMcpToolCall(tools, "deploy_target", {
        target: { mode: "preview" },
      ...
      "Invalid value for deploy_target.target: did not match any allowed schema",
    );
  });
  ...
  it("validates dynamic client MCP conditional schemas", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "send_notice", {
        channel: "email",
  ...
  it("validates dynamic client MCP pattern properties", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "run_with_env", {
        command: "npm run build",
    ...
    expect(
      ...
      "Invalid value for run_with_env.env.VITE_API_URL: expected at least 1 character(s)",
    );
  });
  ...
  it("validates dynamic client MCP object key constraints", () => {
    ...
    expect(
      ...
      "Invalid value for configure_env.settings: expected at least 1 property",
    );
    expect(
      validateClientMcpToolCall(tools, "configure_env", {
        settings: {
      ...
      "Invalid value for configure_env.settings: expected at most 2 properties",
    );
    expect(
      validateClientMcpToolCall(tools, "configure_env", {
    ...
    expect(
      ...
      "Invalid value for configure_env.settings.APP_SECRET: schema disallows value",
    );
    expect(
      validateClientMcpToolCall(tools, "configure_env", {
        settings: { APP_URL: "" },
      ...
      "Invalid value for configure_env.settings.APP_URL: expected at least 1 character(s)",
    );
  });
  ...
  it("validates nested dynamic client MCP schemas before accepting forwarding calls", () => {
    ...
    expect(
      validateClientMcpToolCall(tools, "call_mcp_tool", {
    ...
    expect(
      ...
      "Invalid value for call_mcp_tool.input.metadata.tags[0]: expected string",
    );
    expect(
      validateClientMcpToolCall(tools, "call_mcp_tool", {
        serverName: "filesystem",
  ...
  it("does not fake a forwarded MCP result when the bridge callback is unavailable", () => {
    ...
    expect(response.error.message).toContain(
      "Outer client callback unavailable",
    );
  });
  ...
  it("posts forwarded MCP tool calls to the bridge callback before returning success", async () => {
    ...
    const child = spawn(
      process.execPath,
      [bridgeScriptPath, "--client-mcp-server"],
      {
  ...
  it("keeps exact custom harness MCP tools available in the subcommand server", async () => {
    ...
    const tools = clientMcpToolDefinitions([
      {
        name: "mcp__github__create_issue",
        description:
          "Create a GitHub issue through the outer harness MCP server.",
        parameters: {
          type: "object",
          properties: {
            owner: { type: "string" },
    ...
    expect(
      listResponse.result.tools.some(
        (tool) => tool.name === "mcp__github__create_issue",
      ),
    ).toBe(true);
    expect(callResponse.result.content[0].text).toBe(
      "FORWARDED_TO_OUTER_CLIENT",
    );
    expect(observedRequest).toEqual({
      url: "/client-tool-call",
      authorization: "Bearer bridge-token",
  ...
  it("tells the SDK to forward compatible client tools through MCP", () => {
    ...
    expect(prompt).toContain(
      'Use SDK mcp with providerIdentifier "client" for every local operation',
    );
    expect(prompt).toContain("client_shell");
    expect(prompt).toContain(
      "Prefer exact client tools and dedicated client MCP tools",
    );
    expect(prompt).toContain("LOCAL TOOL RESULT records are present");
    expect(prompt).toContain(
      "emit exactly one client MCP forwarding tool call and no prose",
    );
  });
  ...
  it("keeps SDK session affinity stable while sending client tools through MCP", () => {
    ...
    expect(
      dynamicSendOptions.mcpServers.client.env
        .CURSOR_SDK_BRIDGE_AGENT_CACHE_KEY,
    ).toEqual(
      baseSendOptions.mcpServers.client.env.CURSOR_SDK_BRIDGE_AGENT_CACHE_KEY,
    );
    expect(
      baseSendOptions.mcpServers.client.env.CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON,
    ).toContain("webfetch");
    expect(
      dynamicSendOptions.mcpServers.client.env
        .CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON,
    ).toContain("webfetch");
    expect(
      dynamicSendOptions.mcpServers.client.env
        .CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON,
    ).toContain("probe_write_file");
  });
});
